...
public class UPMPDefaultsGui extends AbstractConfigGui {
    ...
    public void modifyTestElement(TestElement config) {
        ...
        if (imageParser.isSelected()) {
            config.setProperty(new BooleanProperty(UPMPSamplerBase.IMAGE_PARSER, true));
            enableConcurrentDwn(true);
        } else {
            config.removeProperty(UPMPSamplerBase.IMAGE_PARSER);
            enableConcurrentDwn(false);
            
        }
        if (concurrentDwn.isSelected()) {
            config.setProperty(new BooleanProperty(UPMPSamplerBase.CONCURRENT_DWN, true));
        } else {
            ...
            config.removeProperty(UPMPSamplerBase.CONCURRENT_DWN);
        }
        if(!StringUtils.isEmpty(concurrentPool.getText())) {
        	config.setProperty(new StringProperty(UPMPSamplerBase.CONCURRENT_POOL,
        			concurrentPool.getText()));
        } else {
        	config.setProperty(new StringProperty(UPMPSamplerBase.CONCURRENT_POOL,
        			String.valueOf(UPMPSamplerBase.CONCURRENT_POOL_SIZE)));
        }
        if (!StringUtils.isEmpty(embeddedRE.getText())) {
            config.setProperty(new StringProperty(UPMPSamplerBase.EMBEDDED_URL_RE,
                    embeddedRE.getText()));
        } else {
            config.removeProperty(UPMPSamplerBase.EMBEDDED_URL_RE);
        }
    }
    ...
    public void configure(TestElement el) {
        ...
        imageParser.setSelected(((AbstractTestElement) el).getPropertyAsBoolean(UPMPSamplerBase.IMAGE_PARSER));
        concurrentDwn.setSelected(((AbstractTestElement) el).getPropertyAsBoolean(UPMPSamplerBase.CONCURRENT_DWN));
        concurrentPool.setText(((AbstractTestElement) el).getPropertyAsString(UPMPSamplerBase.CONCURRENT_POOL));
        embeddedRE.setText(((AbstractTestElement) el).getPropertyAsString(UPMPSamplerBase.EMBEDDED_URL_RE, ""));
    }
